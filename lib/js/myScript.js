...
headerListLink.click(function(){
    ...
    if($thispParent.attr('value') === "1") {
        ...
        $thisulParent.css("transition","opacity 0.4s, height 0.4s").css("opacity", "1").css("pointer-events", "auto").css("height","80px");
    } else if($thispParent.attr('value') === "2"){
        $thisiParent.css("transform","rotate(0deg)");
        $thispParent.attr('value','1').removeClass("selected");
        $thisulParent.css("transition","opacity 0.2s, height 0.4s").css("opacity", "0").css("pointer-events", "none").css("height","42px");
    }
});
...
function checkAccount(){
    if(account.valid == true){
        if($(document).find("title").text() != "PyGoJs"){var linking = "../";}else{var linking = "";}
