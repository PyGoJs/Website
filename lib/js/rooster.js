...
var classesfull = ["GNU/Linux","Helpdesk","Project","Cisco","Database","Microsoft Certified Solutions Expert","Sport","Engels","Nederlands","Coaching","Office","Programmeren","Hardware Software","Embedded Systems","Webdesign","Rekenen"];
...
function setSchedule(klasid){
    $.ajax({
       url: "http://xedule.novaember.com/weekschedule.14327.json?year=2015&week="+week+"&refresh=true",
        success: function(){
            $.ajax({
                            ...
                            while (b < 2) {
                                if (data0[x].events[i].start == times[a]) {
                                    ...
                                    if (data0[x].events[i].facilities[0] != null && data0[x].events[i].facilities[0] != null) {
                                        roosterClasses[0].innerHTML += "<div data-day='" + x + "' data-start='" + data0[x].events[i].start + "' data-staff='" + data0[x].events[i].staffs[0] + "' class='classblock' value='" + classID + "' onclick='clickClass(this, " + x + "," + i + ")' style='height: " + positionHeight[e] + "px; top: " + positionTop[a] + "px; left: " + ((x + 1) * positionLeft) + "px; width: "+blockWidth+"px'>" + data0[x].events[i].description + "<br/>" + "<i>" + data0[x].events[i].facilities[0] + " - " + data0[x].events[i].staffs[0] + "</i>" + "<div class='aanwezigheidTab'></div></div>";
                                    } else if (data0[x].events[i].staffs[0] != null && data0[x].events[i].facilities[0] == null) {
                                        roosterClasses[0].innerHTML += "<div data-day='" + x + "' data-start='" + data0[x].events[i].start + "' data-staff='" + data0[x].events[i].staffs[0] + "' class='classblock' value='" + classID + "' onclick='clickClass(this, " + x + "," + i + ")' style='height: " + positionHeight[e] + "px; top: " + positionTop[a] + "px; left: " + ((x + 1) * positionLeft) + "px; width: "+blockWidth+"px'>" + data0[x].events[i].description + "<br/>" + "<i>" + data0[x].events[i].staffs[0] + "</i>" + "<div class='aanwezigheidTab'></div></div>";
                                    } else if (data0[x].events[i].staffs[0] == null && data0[x].events[i].facilities[0] != null) {
                                        roosterClasses[0].innerHTML += "<div data-day='" + x + "' data-start='" + data0[x].events[i].start + "' class='classblock' value='" + classID + "' onclick='clickClass(this, " + x + "," + i + ")' style='height: " + positionHeight[e] + "px; top: " + positionTop[a] + "px; left: " + ((x + 1) * positionLeft) + "px; width: "+blockWidth+"px'>" + data0[x].events[i].description + "<br/>" + "<i>" + data0[x].events[i].facilities[0] + "</i>" + "<div class='aanwezigheidTab'></div></div>";
                                    } else {
                                        roosterClasses[0].innerHTML += "<div data-day='" + x + "' data-start='" + data0[x].events[i].start + "' class='classblock' value='" + classID + "' onclick='clickClass(this, " + x + "," + i + ")' style='height: " + positionHeight[e] + "px; top: " + positionTop[a] + "px; left: " + ((x + 1) * positionLeft) + "px; width: "+blockWidth+"px'>" + data0[x].events[i].description + "<br/>" + "<i></i>" + "<div class='aanwezigheidTab'></div></div>";
                                    }
                                } else {
                                    a++;
                                }
...
function clickClass(e, x,i){
    ...
    if($("header ul#pcNav").css("display") == "none"){
        ...
        document.getElementById("bucket").innerHTML += "<div id='popup'><div id='block'><div id='topbar'>"+lesNaam+"<i onclick='closeClass()' class='fa fa-times'></i></div>"+data0[x].events[i].description+"<div class='aanwezigen'></div>"+"</div></div>";
