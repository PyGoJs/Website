...
function checkSession(){
    var session = getCookie("session");
        ...
        $.post( "http://pygojs.remi.im/api/attendee?authkey="+session+"&ciid=32", function( data ) {
            klasdata = data;
            if(klasdata.error == null){
                $("#pcNav li#account p a")[0].innerText = "Account";
            } else {
                $("#pcNav li#account p a")[0].innerText = "Login";
            }
            checkAccount();
        });
